...
def backtest(df, initial_capital, fee_rate, tax_rate):
    ...
    metrics = {
        ...
        '平均每筆手續費': f"{fees.mean():.3f}元",
        '獲利交易次數': f"{win_mask.sum()}次",
        '虧損交易次數': f"{loss_mask.sum()}次",
        '勝率': f"{(win_mask.sum() / len(trades))*100:.2f}%",
        '平均獲利金額': f"{trade_profits[win_mask].mean() if win_mask.any() else 0:.3f}元",
        '平均虧損金額': f"{trade_profits[loss_mask].mean() if loss_mask.any() else 0:.3f}元",
